...
async def monitor_events(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not w3 or not contract:
        logger.warning("Web3 or contract not initialized, skipping monitor_events")
        return
    try:
        current_block = await w3.eth.block_number
        if current_block > last_processed_block:
            from_block = last_processed_block + 1
            events = await get_purchase_events(None, from_block, current_block)
            async with pool.acquire() as conn:
                for event in events:
                    buyer = event.args.buyer
                    if buyer in reverse_sessions:
                        user_id = reverse_sessions[buyer]
                        await conn.execute(
                            "INSERT INTO purchases (user_id, wallet_address, location_id, timestamp) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING",
                            user_id, buyer, event.args.locationId, event.args.timestamp
                        )
                        await context.bot.send_message(
                            user_id,
                            f"You purchased climb #{event.args.locationId}! View with /viewclimb {event.args.locationId} or /mypurchases. 🧗"
                        )
            last_processed_block = current_block
            logger.info(f"Processed events from {from_block} to {current_block}")
    except Exception as e:
        logger.error(f"Error in monitor_events: {str(e)}")

async def startup_event():
    start_time = time.time()
    global application, webhook_failed, pool, sessions, reverse_sessions, pending_wallets, journal_data, last_processed_block
    try:
        ...
        if w3 and contract:
            ...
            logger.info(f"Backfill complete: Processed {len(all_events)} events, set last_processed_block to {last_processed_block}")
        ...
        application.add_handler(MessageHandler(filters.COMMAND, debug_command))  # Changed to debug_command
